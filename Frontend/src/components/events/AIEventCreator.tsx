...
export default function AIEventCreator({ onEventGenerated }: AIEventCreatorProps) {
  ...
  const generateEvent = async () => {
    ...
    setLoading(true);
    try {
      const response = await eventAPI.generateEventAI(prompt);
      onEventGenerated(response.data);
      toast.success("Event details generated! Review and customize below.");
      setIsOpen(false);
      setPrompt("");
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Failed to generate event. Please try again.");
      console.error(error);
    } finally {
      setLoading(false);
    }
