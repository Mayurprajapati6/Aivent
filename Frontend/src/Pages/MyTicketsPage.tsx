...
export default function MyTicketsPage() {
  ...
  const upcomingTickets = registrations.filter(
    (reg) =>
      reg.event &&
      new Date((reg.event as any).startDate).getTime() >= now &&
      reg.status === "confirmed"
  );
  const pastTickets = registrations.filter(
    (reg) =>
      reg.event &&
      (new Date((reg.event as any).startDate).getTime() < now || reg.status === "cancelled")
  );
            ...
            <div className="space-y-4">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  {(selectedTicket.event as any).title}
                </p>
              </div>
              ...
              <div className="bg-muted p-4 rounded-lg space-y-2 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {format(new Date((selectedTicket.event as any).startDate), "PPP, h:mm a")}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <MapPin className="w-4 h-4" />
                  <span>
                    {(selectedTicket.event as any).locationType === "online"
                      ? "Online Event"
                      : `${(selectedTicket.event as any).city}, ${
                          (selectedTicket.event as any).state || (selectedTicket.event as any).country
                        }`}
                  </span>
                </div>
              </div>
